...
const ArtistListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/artist/view/${data.artistid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/artist/edit/${data.artistid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.artistid) },
			icon: "pi pi-trash"
		}
	]
	...
	return (
<main id="ArtistListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('artist')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/artist/add`}>
                        <Button label={$t('addNewArtist')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
...
ArtistListPage.defaultProps = {
	primaryKey: 'artistid',
	pageName: 'artist',
	apiPath: 'artist/index',
