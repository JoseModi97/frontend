...
const InvoicelineListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: $t('search'),
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/invoiceline/view/${data.invoicelineid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/invoiceline/edit/${data.invoicelineid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
	...
	function InvoiceidTemplate(data){
		if(data){
			return (
				<>{data.invoiceid && <Button className="p-button-text" icon="pi pi-eye" label={$t('invoice')} onClick={() => app.openPageDialog(<InvoiceViewPage isSubPage apiPath={`/invoice/view/${data.invoiceid}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	function TrackidTemplate(data){
		if(data){
			return (
				<>{data.trackid && <Button className="p-button-text" icon="pi pi-eye" label={$t('track')} onClick={() => app.openPageDialog(<TrackViewPage isSubPage apiPath={`/track/view/${data.trackid}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	return (
<main id="InvoicelineListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('invoiceline')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/invoiceline/add`}>
                        <Button label={$t('addNewInvoiceline')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="invoiceid" header={$t('invoiceid')} body={InvoiceidTemplate}  ></Column>
                                    <Column  field="trackid" header={$t('trackid')} body={TrackidTemplate}  ></Column>
...
InvoicelineListPage.defaultProps = {
	primaryKey: 'invoicelineid',
	pageName: 'invoiceline',
	apiPath: 'invoiceline/index',
	routeName: 'invoicelinelist',
