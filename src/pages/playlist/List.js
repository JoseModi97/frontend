...
const PlaylistListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/playlist/view/${data.playlistid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/playlist/edit/${data.playlistid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
	...
	return (
<main id="PlaylistListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('playlist')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/playlist/add`}>
                        <Button label={$t('addNewPlaylist')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
...
PlaylistListPage.defaultProps = {
	...
	pageName: 'playlist',
	apiPath: 'playlist/index',
	routeName: 'playlistlist',
