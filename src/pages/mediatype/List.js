...
const MediatypeListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/mediatype/view/${data.mediatypeid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/mediatype/edit/${data.mediatypeid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.mediatypeid) },
			icon: "pi pi-trash"
		}
	]
	...
	return (
<main id="MediatypeListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('mediatype')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/mediatype/add`}>
                        <Button label={$t('addNewMediatype')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
...
MediatypeListPage.defaultProps = {
	primaryKey: 'mediatypeid',
	pageName: 'mediatype',
	apiPath: 'mediatype/index',
	routeName: 'mediatypelist',
...
export default MediatypeListPage;
