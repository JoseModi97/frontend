...
const UserListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/user/view/${data.id}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/user/edit/${data.id}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
	...
	function ProfileTemplate(data){
		if(data){
			return (
				<><ImageViewer imageSize="small" previewSize="" src={data.profile} width="50px" height="50px" numDisplay={1} style={{maxWidth:'100%'}} /></>
			);
		}
	}
	...
	return (
<main id="UserListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('user')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="telephone" header={$t('telephone')} body={TelephoneTemplate}  ></Column>
                                    <Column  field="profile" header={$t('profile')} body={ProfileTemplate}  ></Column>
                                    <Column  field="otp_code" header={$t('otpCode')}   ></Column>
                                    <Column  field="otp_date" header={$t('otpDate')}   ></Column>
