...
import EmployeeViewPage from 'pages/employee/View';
...
const CustomerListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/customer/view/${data.customerid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/customer/edit/${data.customerid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.customerid) },
			icon: "pi pi-trash"
		}
	]
	...
	function SupportrepidTemplate(data){
		if(data){
			return (
				<>{data.supportrepid && <Button className="p-button-text" icon="pi pi-eye" label={$t('employee')} onClick={() => app.openPageDialog(<EmployeeViewPage isSubPage apiPath={`/employee/view/${data.supportrepid}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	return (
<main id="CustomerListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('customer')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/customer/add`}>
                        <Button label={$t('addNewCustomer')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="state" header={$t('state')}   ></Column>
                                    <Column  field="country" header={$t('country')}   ></Column>
                                    <Column  field="postalcode" header={$t('postalcode')}   ></Column>
                                    <Column  field="phone" header={$t('phone')} body={PhoneTemplate}  ></Column>
                                    <Column  field="fax" header={$t('fax')}   ></Column>
                                    <Column  field="email" header={$t('email')} body={EmailTemplate}  ></Column>
                                    <Column  field="supportrepid" header={$t('supportrepid')} body={SupportrepidTemplate}  ></Column>
