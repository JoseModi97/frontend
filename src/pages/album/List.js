...
import ArtistViewPage from 'pages/artist/View';
...
const AlbumListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/album/view/${data.albumid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/album/edit/${data.albumid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.albumid) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function ArtistidTemplate(data){
		if(data){
			return (
				<>{data.artistid && <Button className="p-button-text" icon="pi pi-eye" label={$t('artist')} onClick={() => app.openPageDialog(<ArtistViewPage isSubPage apiPath={`/artist/view/${data.artistid}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	return (
<main id="AlbumListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('album')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/album/add`}>
                        <Button label={$t('addNewAlbum')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="artistid" header={$t('artistid')} body={ArtistidTemplate}  ></Column>
