...
const InvoiceListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/invoice/view/${data.invoiceid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/invoice/edit/${data.invoiceid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.invoiceid) },
			icon: "pi pi-trash"
		}
	]
		return (<SplitButton dropdownIcon="pi pi-bars" className="dropdown-only p-button-text p-button-plain" model={items} />);
	}
	function CustomeridTemplate(data){
		if(data){
			return (
				<>{data.customerid && <Button className="p-button-text" icon="pi pi-eye" label={$t('customer')} onClick={() => app.openPageDialog(<CustomerViewPage isSubPage apiPath={`/customer/view/${data.customerid}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	return (
<main id="InvoiceListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('invoice')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/invoice/add`}>
                        <Button label={$t('addNewInvoice')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="customerid" header={$t('customerid')} body={CustomeridTemplate}  ></Column>
                                    <Column  field="invoicedate" header={$t('invoicedate')}   ></Column>
                                    <Column  field="billingaddress" header={$t('billingaddress')}   ></Column>
                                    <Column  field="billingcity" header={$t('billingcity')}   ></Column>
                                    <Column  field="billingstate" header={$t('billingstate')}   ></Column>
                                    <Column  field="billingcountry" header={$t('billingcountry')}   ></Column>
                                    <Column  field="billingpostalcode" header={$t('billingpostalcode')}   ></Column>
...
InvoiceListPage.defaultProps = {
	primaryKey: 'invoiceid',
	pageName: 'invoice',
	apiPath: 'invoice/index',
