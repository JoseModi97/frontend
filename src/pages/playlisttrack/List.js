...
const PlaylisttrackListPage = (props) => {
		const app = useApp();
	const filterSchema = {
		search: {
			tagTitle: $t('search'),
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/playlisttrack/view/${data.playlistid}`) },
			icon: "pi pi-eye"
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/playlisttrack/edit/${data.playlistid}`) },
			icon: "pi pi-pencil"
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.playlistid) },
			icon: "pi pi-trash"
		}
	]
	...
	return (
<main id="PlaylisttrackListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('playlisttrack')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <Link to={`/playlisttrack/add`}>
                        <Button label={$t('addNewPlaylisttrack')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                        </Link>
                    </div>
...
PlaylisttrackListPage.defaultProps = {
	primaryKey: 'playlistid',
	pageName: 'playlisttrack',
	apiPath: 'playlisttrack/index',
	routeName: 'playlisttracklist',
...
export default PlaylisttrackListPage;
